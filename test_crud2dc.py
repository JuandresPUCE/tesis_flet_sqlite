...
class PantallaCrud(QMainWindow):
    def __init__(self):
        super(PantallaCrud, self).__init__()
        loadUi("crud_datos_cineticos.ui", self)
        self.DatosCineticosManejador = DatosCineticosManejador()

        # UI elements
        
        self.tiempo = self.tiempo_edit
        self.concentracion = self.concentracion_edit
        self.otra_propiedad = self.otra_propiedad_edit
        self.conversion_reactivo_limitante = self.conversion_reactivo_limitante_edit
        self.tipo_especie = self.tipo_especie_edit
        self.id_condiciones_iniciales = self.id_condiciones_iniciales_edit
        self.nombre_data = self.nombre_data_edit
        self.nombre_reaccion = self.nombre_reaccion_edit
        self.especie_quimica = self.especie_quimica_edit

        # Buttons
        self.agregar_btn = self.agregar_btn
        self.actualizar_btn = self.actualizar_btn
        self.seleccionar_btn = self.selecionar_btn
        self.buscar_btn = self.buscar_btn
        self.limpiar_btn = self.limpiar_btn
        self.borrar_btn = self.borrar_btn

        # Table
        self.tabla_datos = self.tableWidget
        self.tabla_datos.setSortingEnabled(False)
        self.lista_botones = self.funciones_frame.findChildren(QPushButton)
